...
public class PrecentInvestAward implements InviteAwad {
	...
	public PrecentInvestAward(Tender tender) {
		super();
		this.tender = tender;
		this.rule = Global.getRule(EnumRuleNid.PERCENT_AWARD.getValue());
		this.checkModel = JSON.parseObject(rule.getRule_check(), RuleCheckModel.class);
	}
	public PrecentInvestAward instance(){
...
//                AwardGiveInviteInvestLog log=new AwardGiveInviteInvestLog(rs.getReceive_account(),rs.getReward_user_id(),1L);
				...
				bLog.doEvent();
				// 添加奖励记录：好友邀请奖励
				rewardRecordDao.save(EnumRewardStatisticsType.INVITE_AWARD.getValue(), 
	...
	private void newAward() {
		...
		if (!StringUtils.isBlank(tenderUser.getInvite_userid())) {
			...
			rewardStatistics.setType(EnumRewardStatisticsType.INVITER_AWARD
					.getValue()); // 类型：邀请人奖励
			// 邀请人(user.getInvite_userid())的奖励
			rewardStatistics.setReward_user_id(Long.valueOf(tenderUser
					.getInvite_userid()));
			Account tenderAct = accountDao.getAccount(tender.getUser_id());
			if (repayCount == 1) {
				if (tenderAct.getCollection() > checkModel.getTender_check_money()) {
					// 如果待收金额 > 规则里的上线金额，该奖励记录标记为可以显示
					rewardStatistics.setIs_show(1);
					rewardStatistics.setPassive_user_id(tenderUser.getUser_id()); // 会员ID
					rewardStatistics.setReceive_time(DateUtils.getTimeStr(new Date())); // 应收时间
					if (backType== EnumRewardStatisticsBackType.TIME_BACK
							.getValue()) { // 返现方式：定时返现
					...
					if (receiveRate > 0) {
						...
						// 如果规则中直接配置发放多少奖励，奖励=规则中的奖励
						rewardStatistics.setReceive_account(checkModel.getReceive_account());
					}
					...
					rewardStatistics.setEndtime(checkModel.getEnd_time());
